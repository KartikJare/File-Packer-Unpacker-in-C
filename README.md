...
Directory Traversal: Opens the target directory using opendir() and iterates through entries via readdir().

Filtering: Ignores . and .. and ensures only regular files are processed via S_ISREG.

Size Calculation: Calculates file size using lseek to the end of the file.

Serialization: Writes the FileHeader followed by the raw file content into the destination archive.

Unpacking Process (UnpackedFiles)
Header Parsing: Reads the FileHeader from the archive to determine the filename and exact byte count to read.

Restoration: Creates the output directory and opens a new file for writing.

Buffered Read: Reads the content in chunks defined by BUFFER_SIZE (1024 bytes) until the specific file size is reached.

---

⚠️ Limitations
Filename Length: Names are capped at 99 characters due to the FileHeader struct limits.

Non-Recursive: Does not currently pack subdirectories inside the source folder; it only processes regular files.

Permissions: Files are recreated with 0777 permissions rather than preserving original file modes.
